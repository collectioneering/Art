...
C# implementation original author: FireXware

Modified source obtained from:
https://github.com/Lucina/Fp/blob/main/src/Fp.Plus/Ciphers/BlowFish.cs
@28b30fd

Local changes:
-Namespace changed to Art
-Removed scripting parts
-Removed unnecessary (and strange) buffer copy to self in SetKey

Changes:
-Moved to pointers/spans
-Killed unnecessary managed allocations
-Streamlined operations
-Removed functions outside scope of Fp
 */
...
#pragma warning restore 649
    ...
    private const int S0Off = POff + PLen;
    private const int S0Len = 4 * 256;

    private const int S1Off = S0Off + S0Len;
    private const int S1Len = 4 * 256;

    private const int S2Off = S1Off + S1Len;
    private const int S2Len = 4 * 256;

    private const int S3Off = S2Off + S2Len;
    private const int S3Len = 4 * 256;

    private const int IvOff = S3Off + S3Len;
    private const int IvLen = 8;

    private const int MLen = IvOff + IvLen;
    ...
    public unsafe void SetKey(ReadOnlySpan<byte> cipherKey)
    {
        ...
        fixed (byte* b = _stateBuffer, k = &cipherKey.GetPinnableReference())
        {
            uint* p = (uint*)(b + POff),
                s0 = (uint*)(b + S0Off),
                s1 = (uint*)(b + S1Off),
            ...
            *blR = 0;
            ...
            for (int i = 0; i < 18; i++)
            {
                p[i] ^= (uint)((((((k[j % keyLen] << 8) | k[(j + 1) % keyLen]) << 8) |
                                 k[(j + 2) % keyLen]) << 8) | k[(j + 3) % keyLen]);
                j = (j + 4) % keyLen;
            }
    ...
    private static unsafe void BlockDecryptBe(byte* block, uint* p, uint* s0, uint* s1, uint* s2, uint* s3)
    {
        ...
        *blL ^= p[17];
        for (byte i = 16; i > 0; i -= 2)
        {
            *blR ^= (((s0[*block] + s1[block[1]]) ^ s2[block[2]]) +
                     s3[block[3]]) ^ p[i];
            *blL ^= (((s0[block[4]] + s1[block[5]]) ^ s2[block[6]]) +
                     s3[block[7]]) ^ p[i - 1];
        }
