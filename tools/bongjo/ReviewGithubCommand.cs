...
public sealed partial class ReviewGithubCommand : Command
{
    ...
    public ReviewGithubCommand(string name, string? description = null) : base(name, description)
    {
        ...
        Add(_sarifFile);
        _ownerOption = new Option<string>("--owner") { Description = "GitHub repo owner name", Required = true };
        Add(_ownerOption);
        _issueNumberOption = new Option<int>("--issue") { Description = "Issue number in repo", Required = true };
        Add(_issueNumberOption);
        _repoOption = new Option<string>("--repo") { Description = "GitHub repo name", Required = true };
        Add(_repoOption);
    ...
    private async Task<int> HandleAsync(ParseResult parseResult, CancellationToken cancellationToken)
    {
        ...
        string owner = parseResult.GetRequiredValue(_ownerOption);
        int issue = parseResult.GetRequiredValue(_issueNumberOption);
        string repo = parseResult.GetRequiredValue(_repoOption);
        SarifFile sarifFile;
                ...
                foreach (var location in result.Locations)
                {
                    ...
                    if (ranges.TryGetValue(path, out var fileRanges))
                    {
                        ...
                        foreach (var range in fileRanges)
                        {
                            ...
                            {
                                comments.Add(new TargetedComment(localPath, overlapColumn, overlapCount, result.Message.Text, result.RuleId, result.Level));
                            }
                        }
                    }
                }
        ...
        foreach (var comment in comments)
        {
            ...
            await gitHubClient.PullRequest.ReviewComment.Create(owner, repo, issue, new PullRequestReviewCommentCreate(commentText, headCommit, comment.File, comment.Line));
        }
        return 0;
    }

    private record TargetedComment(string File, int Line, int Column, string Message, string RuleId, string Level);

    private record struct FixedRange(int Index, int Length) : IComparable<FixedRange>
    {
